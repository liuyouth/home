<template>
  <div class="app"  >
      ...
      <div class="row" style=" width: 100%;   gap: 28px; justify-content: center;">
        <template v-for="p in pages">
          <div class="pageBox" @click="onPageSelect(p)"  v-bind:id="p.name" >
            <div :class="{'currPage':p.isCurrent}" style="width: 120px;min-height: 90px;background-color:#131518d8;border-radius: 10px;">

            </div>
            <input style="width: 120px;" class="showText" type="text"  placeholder="Page 1"  ref="minel" v-model="p.name" />
          </div>
        </template>
      </div>
    ...
    <div id="widgetsBox" :class="{'menuBoxOpen':isMenuOpen}">

    
    <template v-for="w in pages[currIndex].widgets">
